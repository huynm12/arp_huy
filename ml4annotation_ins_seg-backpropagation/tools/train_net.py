...
def main():
    ...
    if os.path.isfile(config.MODEL.WEIGHTS):
        model_weights = torch.load(config.MODEL.WEIGHTS, map_location='cpu')
        ...
        if os.path.isfile(config.MODEL.BACKBONE.WEIGHTS):
            pretrained_weights = torch.load(config.MODEL.BACKBONE.WEIGHTS, map_location='cpu')
    ...
    if config.TRAIN.RESUME:
        ...
        if os.path.isfile(model_state_file):
            ...
            ########
            # checkpoint['state_dict'] = {k: v for k, v in checkpoint['state_dict'].items() if "semantic_backbone" not in k}
            ########
            get_module(model, distributed).load_state_dict(checkpoint['state_dict'], strict=False)
    ...
    try:
        for i in range(start_iter, max_iter):
            try:
                data_loader.batch_sampler.sampler.set_epoch(i)
            except:
                pass
            ...
            for batch_idx, data in enumerate(data_loader):
                ...
                if batch_idx == 0 or (batch_idx + 1) % config.PRINT_FREQ == 0:
                    msg = '[{0}/{1}][{2}/{3}] LR: {4:.7f}\t' \
                        'Time: {batch_time.val:.3f}s ({batch_time.avg:.3f}s)\t' \
                        'Data: {data_time.val:.3f}s ({data_time.avg:.3f}s)\t'.format(
                            i + 1, max_iter, batch_idx + 1, len(data_loader), lr, batch_time=batch_time, data_time=data_time)
            ...
            if comm.is_main_process():
                ...
                # os.path.join(config.OUTPUT_DIR, 'iter_{}.pth'.format(i+1)))
            lr_scheduler.step()
    except Exception:
