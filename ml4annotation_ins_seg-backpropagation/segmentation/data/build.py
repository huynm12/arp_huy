...
from .datasets import Cityscapes, CityscapesPanoptic, COCOPanoptic, VinAIInstance, VinAIPanoptic
...
def build_dataset_from_cfg(config, is_train=True):
    ...
    dataset_map = {
        ...
        'vinai_instance': VinAIInstance,
        'vinai_panoptic': VinAIPanoptic
    }
    ...
    dataset_cfg = {
        ...
        'vinai_instance': dict(
            root=config.DATASET.ROOT,
            info_file=config.DATASET.TRAIN_FILE if is_train else config.DATASET.TEST_FILE,
            is_train=is_train,
            crop_size=config.DATASET.CROP_SIZE if is_train else config.TEST.CROP_SIZE,
            mirror=config.DATASET.MIRROR,
            min_scale=config.DATASET.MIN_SCALE,
        ...
        'vinai_panoptic': dict(
            ...
            is_train=is_train,
            crop_size=config.DATASET.CROP_SIZE if is_train else config.TEST.CROP_SIZE,
            rescale_size=config.DATASET.RESCALE_SIZE if is_train else config.TEST.RESCALE_SIZE,
            mirror=config.DATASET.MIRROR,
            min_scale=config.DATASET.MIN_SCALE,
            max_scale=config.DATASET.MAX_SCALE,
            scale_step_size=config.DATASET.SCALE_STEP_SIZE,
...
def build_train_loader_from_cfg(config):
    ...
    if sampler_name == "TrainingSampler":
        ...
        sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=config.DATALOADER.TRAIN_SHUFFLE)
    else:
