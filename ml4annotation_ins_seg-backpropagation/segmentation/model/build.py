...
from .meta_arch import DeepLabV3, DeepLabV3Plus, PanopticDeepLab, InstanceDeepLab, VinAIPanopticDeepLab
from .loss import RegularCE, OhemCE, DeepLabCE, L1Loss, MSELoss, CrossEntropyLoss
from segmentation.data.datasets.vinai_panoptic import _THING_IDS
...
def build_segmentation_model_from_cfg(config):
    ...
    model_map = {
        ...
        'instance_deeplab': InstanceDeepLab,
        'vinai_panoptic_deeplab': VinAIPanopticDeepLab
    }
    ...
    model_cfg = {
        ...
        'instance_deeplab': dict(
            ...
            offset_loss=build_loss_from_cfg(config.LOSS.OFFSET),
            offset_loss_weight=config.LOSS.OFFSET.WEIGHT,
            in_semantic_channels=config.DATASET.NUM_CLASSES if config.MODEL.PANOPTIC_DEEPLAB.INSTANCE.ORACLE_SEMANTIC else None,
            out_semantic_channels=config.MODEL.PANOPTIC_DEEPLAB.INSTANCE.SEMANTIC_EMBED_CHANNELS if config.MODEL.PANOPTIC_DEEPLAB.INSTANCE.ORACLE_SEMANTIC else None,
        ),
        'vinai_panoptic_deeplab': dict(
            replace_stride_with_dilation=config.MODEL.BACKBONE.DILATION,
            in_channels=config.MODEL.DECODER.IN_CHANNELS,
            feature_key=config.MODEL.DECODER.FEATURE_KEY,
            low_level_channels=config.MODEL.PANOPTIC_DEEPLAB.LOW_LEVEL_CHANNELS,
            ...
            offset_loss_weight=config.LOSS.OFFSET.WEIGHT,
            out_semantic_channels=config.MODEL.PANOPTIC_DEEPLAB.INSTANCE.SEMANTIC_EMBED_CHANNELS if config.MODEL.PANOPTIC_DEEPLAB.INSTANCE.ORACLE_SEMANTIC else None,
            pretrained_semantic=config.MODEL.PANOPTIC_DEEPLAB.SEMANTIC.PRETRAINED,
            ignore_semantic_head=config.MODEL.PANOPTIC_DEEPLAB.SEMANTIC.IGNORE_HEAD,
            ...
            has_instance=config.MODEL.PANOPTIC_DEEPLAB.INSTANCE.ENABLE,
            freeze_module=config.MODEL.PANOPTIC_DEEPLAB.FREEZE_MODULE,
            use_semantic=config.MODEL.PANOPTIC_DEEPLAB.INSTANCE.USE_SEMANTIC
        ),
    }
