...
__all__ = ['find_instance_center', 'get_instance_segmentation', 'get_panoptic_segmentation', 'get_inconsistency_map', 'get_instance_weight_map']
...
def get_instance_segmentation(sem_seg, ctr_hmp, offsets, thing_list, threshold=0.1, nms_kernel=3, top_k=None,
    ...
    ### NEW POST-PROCESSING
    thing_seg = torch.zeros_like(sem_seg)
    for idx, thing_class in enumerate(thing_list):
        thing_seg[sem_seg == thing_class] = idx + 1
    
    final_ctr = []
    final_ins_seg = torch.zeros_like(sem_seg)
    max_instance_id = 0
    ...
    for thing_id in thing_seg.unique():
        if thing_id == 0: continue
        
        thing_reg = thing_seg == thing_id
        curr_offsets = offsets * thing_reg
        ...
        if len(curr_ctr) == 0: continue
        curr_ctr = torch.tensor(curr_ctr, device=ctr.device)
        final_ctr += [curr_ctr]

        ins_seg = group_pixels(curr_ctr, curr_offsets)
        ins_seg = ins_seg * thing_reg
        ins_seg[ins_seg > 0] += max_instance_id

        final_ins_seg += ins_seg
        max_instance_id += curr_ctr.size(0)
    
    if len(final_ctr) == 0:
        return torch.zeros_like(sem_seg), ctr.unsqueeze(0)
    final_ctr = torch.cat(final_ctr, dim=0)
    return final_ins_seg, final_ctr.unsqueeze(0)
...
def get_inconsistency_map(sem, ctr_hmp, offsets, thing_list,
                              threshold=0.1, nms_kernel=3, top_k=None, foreground_mask=None):
    ...
    inconsistency_map = torch.zeros_like(semantic)
    for thing_class in thing_list:
    ...
    for ins_id in instance_ids:
        ...
        if torch.nonzero(thing_mask).size(0) == 0:
            continue
        instance_sem = semantic[thing_mask].view(-1, )
        major_class_id, _ = torch.mode(instance_sem)
        
        for class_id in instance_sem.unique():
            if class_id == major_class_id: continue
            inconsistency_map[thing_mask & (semantic == class_id)] = 1
    
    return inconsistency_map

def get_instance_weight_map(sem, sem_gt, ctr_hmp, offsets, thing_list,
                              threshold=0.1, nms_kernel=3, top_k=None, foreground_mask=None):
    if sem.dim() != 4 and sem.dim() != 3:
        raise ValueError('Semantic prediction with un-supported dimension: {}.'.format(sem.dim()))
    if sem.dim() == 4 and sem.size(0) != 1:
    ...
    # In case thing mask does not align with semantic prediction
    semantic_thing_seg = torch.zeros_like(semantic)
    instance_weight_map = torch.zeros_like(semantic).float()
    for thing_class in thing_list:
        semantic_thing_seg[semantic == thing_class] = 1
    ...
    for ins_id in instance_ids:
        ...
        # Analyze frequency of instance
        instance_sem_pred = semantic[thing_mask].view(-1, )
        instance_sem_gt = sem_gt[thing_mask].view(-1, )
        existed_classes = set(instance_sem_pred.unique().cpu().numpy().tolist() + instance_sem_gt.unique().cpu().numpy().tolist())
        ce_val = 0
        area = instance_sem_pred.size(0)
        eps = 1e-7
        for class_id in existed_classes:
            y = (instance_sem_gt == class_id).sum() / area
            y_hat = (instance_sem_pred == class_id).sum() / area + eps
            ce_val -= y * torch.log(y_hat)
        instance_weight_map[thing_mask] = ce_val
    # import pdb; pdb.set_trace()
    return instance_weight_map