...
class OhemCE(nn.Module):
    ...
    def forward(self, score, target, **kwargs):
        ...
        if 'semantic_weights' in kwargs and kwargs['semantic_weights'] is not None:
            pixel_losses = self.criterion(score, target) * kwargs['semantic_weights']
            pixel_losses = pixel_losses.contiguous().view(-1)
        else:
        ...
        if pixel_losses[pixel_losses > 0].numel() > 0:
            return pixel_losses.sum() / pixel_losses[pixel_losses > 0].numel()
        else:
