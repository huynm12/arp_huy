...
class Resize(object):
    ...
    def __call__(self, image, label):
        ...
        image = cv2.resize(image.astype(np.float), self.size[::-1], interpolation=cv2.INTER_LINEAR)
        label = cv2.resize(label.astype(np.float), self.size[::-1], interpolation=cv2.INTER_NEAREST)
