...
class VinAIPanopticTargetGenerator(object):
    """
    ...
    Arguments:
        ...
        thing_list: List, a list of thing class names
    ...
    """
    def __init__(self, num_labels, ignore_label, sigma=8,
    ...
    def __call__(self, panoptic, metas):
        """Generates the training target.
        Args:
            panoptic: numpy.array, grayscale image encoding classes.
            metas: list, list of dictionary containing information of classes:
                - name: class name
                - index: index of instance.
                - color: hex string color.
                - attributes: dict, attribute of instance.
        ...
        foreground_semantic = np.zeros((self.num_labels, height, width), dtype=np.uint8)
        for meta in metas:
            
            semantic[panoptic == meta["index"]] = meta["sem_id"]
            
            if "ins_id" not in meta: continue
            foreground[panoptic == meta["index"]] = 1
            foreground_semantic[meta["ins_id"] - 1][panoptic == meta["index"]] = 1

            # Ignored regions are not in `segments`.
            # Handle crowd region.
            center_weights[panoptic == meta["index"]] = 1
            offset_weights[panoptic == meta["index"]] = 1

            # find instance center
            mask_index = np.where(panoptic == meta["index"])
            ...
            if ins_area < self.small_instance_area:
                semantic_weights[panoptic == meta["index"]] = self.small_instance_weight
        ...
        return dict(
            ...
            foreground_semantic=torch.as_tensor(foreground_semantic.astype(np.float32)),
...
class InstanceTargetGenerator(object):
    ...
    def __call__(self, panoptic, metas):
        ...
        for meta in metas:
            foreground[panoptic == meta["index"]] = 1
            semantic[panoptic == meta["index"]] = meta["cat_id"]
            foreground_semantic[meta["cat_id"] - 1][panoptic == meta["index"]] = 1
